...
let play_album album_href =
  with_proxy_return_ok (fun proxy ->
    Spotify.stop proxy >>= fun () ->
    Spotify.open_uri proxy album_href)

let play_artist artist_href =
  with_proxy_return_ok (fun proxy ->
    Spotify.stop proxy >>= fun () ->
    Spotify.open_uri proxy artist_href)

let play_track track_href =
  with_proxy_return_ok (fun proxy ->
    Spotify.stop proxy >>= fun () ->
    Spotify.open_uri proxy track_href)
